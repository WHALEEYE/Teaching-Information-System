...
public final class ProjectJudge {
  ...
  private final DepartmentService departmentService = serviceFactory
      .createService(DepartmentService.class);
  private final SemesterService semesterService = serviceFactory
      .createService(SemesterService.class);
  ...
  public EvalResult testSearchCourses(File searchCourseDir) {
    ...
    for (File file : searchCourseDir.listFiles((dir, name) -> !name.endsWith("Result.json"))) {
      ...
      List<List<CourseSearchEntry>> searchCourseResult = IntStream
          .range(0, searchCourseParams.size()).parallel()
  ...
  public void benchmark() {
    ...
    Map<String, List<String>> majorCompulsoryCourses = readValueFromFile(
        "majorCompulsoryCourses.json", Map.class);
    Map<String, List<String>> majorElectiveCourses = readValueFromFile("majorElectiveCourses.json",
        Map.class);
    Map<String, Prerequisite> coursePrerequisites = readValueFromFile("coursePrerequisites.json",
        Map.class);
    Map<String, Map<String, List<CourseSection>>> sections = readValueFromFile(
        "courseSections.json", Map.class);
    Map<String, List<CourseSectionClass>> classes = readValueFromFile("courseSectionClasses.json",
        Map.class);
    Map<String, Map<String, Grade>> studentCourses = readValueFromFile("studentCourses.json",
        Map.class);
    ...
    endTimeNs = System.nanoTime();
    System.out
        .printf("Import student courses time: %.2fs\n", (endTimeNs - startTimeNs) / 1000000000.0);
  ...
  private static <T> T readValueFromFile(File file, Class<T> tClass) {
    ...
    objectMapper
        .enableDefaultTypingAsProperty(ObjectMapper.DefaultTyping.JAVA_LANG_OBJECT, "@type");
    try {
