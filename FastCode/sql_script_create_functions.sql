...
create or replace function get_section_table(i_stu_id integer, i_date date) returns SETOF record
    language plpgsql
as
$$
declare
...
create or replace function get_enrolled_conflict_sections(stu_id integer, sem_id integer) returns SETOF record
    language plpgsql
as
$$
begin
...
-- order by full name
    ...
    drop table if exists student_selected_section_in_same_semester;
    create temp table if not exists student_selected_section_in_same_semester on commit drop as
    select section_id stu_sec_id, full_name stu_full_name
    from (select section_id
          from student_section_relations
          where student_id = stu_id
         ) as ssr
    ...
    drop table if exists student_selected_section_time;
    create temp table if not exists student_selected_section_time on commit drop as
    select c.full_name, week_list, day_of_week, class_begin, class_end
    from course_section_class
    join course_section c on course_section_class.section_id = c.id
    where section_id in (select stu_sec_id
                         from student_selected_section_in_same_semester);
...
-- Course Conflict contains itself
    return query
        select sssiss.stu_full_name, cs.id
        from course_section cs
        join student_selected_section_in_same_semester as sssiss
        on (select course_section.course_id
            from course_section
            where course_section.id = sssiss.stu_sec_id) = cs.course_id
        where semester_id = sem_id

        union

-- Time Conflict contains itself
        select ssst.full_name, assst.section_id
        from student_selected_section_time ssst
                 join all_same_semester_section_time assst
                      on ssst.week_list & assst.week_list > 0
                          and ssst.day_of_week = assst.day_of_week
...
create or replace function search_course(search_student_id integer, search_semester_id integer, search_course_id character varying, search_name character varying, search_instructor character varying, search_day_of_week integer, search_class_time integer, search_class_location character varying[], search_course_type integer, ignore_full boolean, ignore_conflict boolean, ignore_passed boolean, ignore_missing_prerequisites boolean, page_size integer, page_index integer) returns SETOF record
    language plpgsql
as
$$
declare
...
begin
    drop table if exists conflict_sections;
    create temp table if not exists conflict_sections on commit drop as
        (select *
         from get_all_conflict_sections(search_student_id,
                                        search_semester_id) as (section_id int));

    drop table if exists all_conflicts_table;
    create temp table if not exists all_conflicts_table on commit drop as
        (select *
         from get_enrolled_conflict_sections(search_student_id,
                                             search_semester_id) as (e_full_name varchar, e_sec_id integer));

    return query
        with available_sections as (
        ...
        select available_sections.course_id,
               ...
               csc2.class_end,
               csc2.location,
               null                         as conflict_full_name,
               0                            as order_case,
               available_sections.full_name as a_full_name
        ...
        select available_sections.course_id,
               ...
               null,
               null,
               all_conflicts_table.e_full_name as conflict_full_name,
               1                               as order_case,
               available_sections.full_name    as a_full_name
        from available_sections
                 join all_conflicts_table
                      on available_sections.section_id = all_conflicts_table.e_sec_id
        order by course_id, a_full_name,
                 order_case, conflict_full_name;
end;
$$;

alter function search_course(integer, integer, varchar, varchar, varchar, integer, integer, character varying[], integer, boolean, boolean, boolean, boolean, integer, integer) owner to postgres;
...
create or replace function generate_instructor_full_name() returns trigger
    language plpgsql
as
$$
begin
...
create or replace function generate_student_full_name() returns trigger
    language plpgsql
as
$$
begin
